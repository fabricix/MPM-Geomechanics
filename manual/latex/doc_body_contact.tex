...
\begin{algorithm}[H]
	\caption{MPM Multi-body Contact Algorithm}
	\label{alg:mpm_contact}
	\begin{algorithmic}[1]
		
		\For{each grid node $i$}
		\State Identify the bodies that have material points mapped to node $i$
		\State Reconstruct nodal mass $m_i$ and momentum $p_i$, distinguishing contact nodes from regular nodes
		
		\If{node $i$ is a grid contact node}
		\State Compute the external normal relative to each body
		\EndIf
		
		\State Impose essential boundary conditions on grid nodal momentum
		\State Compute grid nodal internal, external, and total forces ($f^{\text{int}}, f^{\text{ext}}, f^{\text{tot}}$)
		\State Integrate the trial momentum of each body independently (without contact)
		\EndFor
		
		\For{each grid contact node $i$}
		\If{body penetration is detected}
		\State Calculate the normal and tangential contact forces
		\State Correct the contact grid nodal momentum
		\EndIf
		\EndFor
		
		\State Update the position $x_{ip}$ and velocity $v_{ip}$ of each body material point
	\end{algorithmic}
\end{algorithm}
...
keyword defines the contact identifier, "friction" specifies the contact friction coefficient, \verb|"master_body_id"| defines the master body 
identifier, \verb|"slave_body_id"| defines the slave body identifier, and \verb|"normal_type"| specifies which normal will be used in the contact analysis.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/single_contact_verification.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/multi_contact_verification.png}
    \caption{Verification of displacements obtained with MPM simulation.}
...
In this example, Bodies 2 and 3 are initially separated by 6 m and 2 m, respectively. In the absence of external forces, they move at constant velocities of 4 m/s and 2 m/s, making contact with Body 1 at t = 1.5 s and t = 1.0 s, respectively.

In a displacement-time graph, Bodies 2 and 3 exhibit linear behavior until contacting Body 1, at which point a trend shift occurs. Graph analysis reveals that omitting the real-distance correction led to contact being detected earlier than predicted by the analytical solution. Conversely, applying the correction yielded contact times that align closely with analytical predictions.

In a velocity-time graph, the bodies maintain a constant velocity until contact, after which their direction of motion reverses. Consistent with the displacement results, the velocity analysis when real-distance corretion is not applied indicates contact even when the gap between the bodies equals the mesh cell size.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/RealDistanceCorrection_Displacement.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/RealDistanceCorrection_Velocity.png}
