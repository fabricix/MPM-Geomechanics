...
Describe here the numerical implementation of the Slave-Master contact algorithm in MPM-Geomechanics.

\begin{lstlisting}
Step:1: For each grid node, identify the bodies that have material points mapped to this node.
Step:2: For each pair of bodies mapped to the same grid node, check the ...
...
\end{lstlisting}

The Slave-Master contact algorithm is implemented in the MPM-Geomechanics code and can be activated by including the following lines in the input file:

\begin{lstlisting}
  Explain here how to include the contact in the input file, e.g.,
